...
public class Black extends AppCompatTextView {

  public Black(Context context, AttributeSet attrs, int defStyle) {
    super(context, attrs, defStyle);
    init();
  }

  public Black(Context context, AttributeSet attrs) {
    super(context, attrs);
    init();
  }
  ...
  public void init() {
    if (Common.getSharedParams(getContext(), Config.SHARAED_PARAMS_language).equals(LANG_FA)){
      Typeface tf = Typeface.createFromAsset(getContext().getAssets(), "fonts/IRANSansMobile_Black.ttf");
      setTypeface(tf ,1);
    }else {
